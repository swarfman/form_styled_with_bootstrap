...
elements.addEventListener ("submit", (e) => {
    ...
    if (zip.value === ""){
     ...
     let state = document.querySelector(".picker");
    if (state.value === "Pick a State"){
        document.querySelector(".picker").style.background = "red";
    }

    let buttonSelector=document.querySelector("#buttongame");
    let ccCard = document.querySelectorAll(".cCardSub");
        ccCard.forEach( (e)=> {
            if (e.checked == true){
           buttonSelector.style.background = "white";
    }
    else{
        return buttonSelector.style.background = "red";
    }
        })
